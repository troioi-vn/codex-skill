...
**Never append `; echo "EXIT=$?"` or any trailing command to the invocation.** The trailing command's own success becomes the command's exit status, so a failed run is reported to your harness as exit 0.
